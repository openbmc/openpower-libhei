...
uint64_t BitString::getField( uint32_t i_pos, uint32_t i_len ) const
{
    ...
    if (i_len - (len0 + len1))
    {
        o_val |= (getField(i_pos + (len0 + len1), i_len - (len0 + len1)));
        o_val <<= UINT8_BIT_LEN;
    }

    // Masks will be the size of the chunk length and right justified
    uint8_t mask0 = static_cast<uint8_t>(-1) >> ( UINT8_BIT_LEN - len0 );
    uint8_t mask1 = static_cast<uint8_t>(-1) >> ( UINT8_BIT_LEN - len1 );
...
void BitString::setField( uint32_t i_pos, uint32_t i_len, uint64_t i_val )
{
    HEI_ASSERT( nullptr != getBufAddr() );      // must to have a valid address
    HEI_ASSERT( 0 < i_len );                    // must have at least one bit
    HEI_ASSERT( i_len <= UINT64_BIT_LEN );      // i_len length must be valid
    ...
    if (i_len - (len0 + len1))
    {
        setField(i_pos + (len0 + len1), i_len - (len0 + len1), i_val >> UINT8_BIT_LEN);
    }
    ...
    *relAddr |= ((i_val & mask0) << relPos); // set destination bits
    ...
    if (len1)
    {
        ...
        *relAddr |= ((i_val & (mask1 << len0)) >> len0); // set detination bits
    }
                            ...
                            uint64_t i_pattern, uint32_t i_pLen )
{
    ...
    for ( uint32_t pos = i_sPos; pos < endPos; pos += i_pLen )
    {
        ...
        uint8_t pattern = bso.getFieldRight( 0, len );
...
//------------------------------------------------------------------------------
// Note: This is shifting the bitstring right in memory (to higher memory
// location) which is like shifitng left in a register. Not obvious.
...
//------------------------------------------------------------------------------
// Note: This is shifting the bitstring left in memory (to lower memory
// location) which is like shifitng right in a register. Not obvious.
BitStringBuffer BitString::operator<<( uint32_t i_shift ) const
{
    BitStringBuffer bsb( getBitLen() ); // default all zeros
