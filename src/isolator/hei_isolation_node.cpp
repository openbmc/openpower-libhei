                             ...
                             IsolationData & io_isoData ) const
{
    ...
    // If this node has already been analyzed, continuing would put us into an
    // infinite loop.
    HEI_ASSERT( ! pushIsolationStack() );
...
void IsolationNode::addChild( uint8_t i_bit,
                                  const IsolationNode * i_child )
{
...
bool IsolationNode::pushIsolationStack() const
{
    // Determine if this node already exists in cv_isolationStack.
    if ( cv_isolationStack.end() != std::find(cv_isolationStack.begin(),
    ...
    {
        // This node does not exist, so push it to cv_isolationStack.
