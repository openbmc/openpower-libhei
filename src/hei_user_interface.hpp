 ...
 *
 * @param i_chip     The target chip for the register access. It is provided
 * to the isolator by the user application via the isolator main APIs.
 *
 * @param o_buffer   Allocated memory space for the returned contents of the
 *                   register.
 *
 * @param io_bufSize The input parameter is the maximum size of the
 * allocated o_buffer. The return value is the number of bytes actually
 *                   written to the buffer.
 *
 * @param i_regType  The user application may support different types of
 *                   registers. This value is provided to the isolator by
 * the user application via the Chip Data Files. The user application is
 * responsible for knowing what to do with this parameter.
 *
 * @param i_address  The register address. The values is a 1, 2, 4, or 8
 * byte address (right justified), which is provided to the isolator by the
 * user application via the Chip Data Files.
 *
 ...
 * to the isolator by the user application via the isolator main APIs.
 *
 * @param i_buffer   Allocated memory space containing the register contents
 * to write to hardware.
 *
 * @param io_bufSize The input parameter is the number of byte from i_buffer
 * to write to the hardware register. The return value is the actual number
 * of bytes written to the hardware register.
 *
 * @param i_regType  The user application may support different types of
 *                   registers. This value is provided to the isolator by
 * the user application via the Chip Data Files. The user application is
