...
/**
 * @brief registerRead reads a register value given a chip type (i_chip),
 *        a register type (i_regType), and an address (i_address).  Also
 *        needed is a pointer to a buffer (o_buffer) to store the data and
 *        a variable (io_bufSize) to pass back its size.
 */
                        ...
                        uint64_t i_regType, uint64_t i_address)
{
    ...
    switch (i_regType)
    {
        ...
        case REG_TYPE_ID_SCOM:
        {
            ...
            io_bufSize = sizeof(regValue);
            memcpy(o_buffer, &regValue, io_bufSize);
            HEI_ERR("ID_registerRead(%p,%p,%lx,%lx,%lx)", i_chip.getChip(),
                    o_buffer, io_bufSize, i_regType, i_address);
            break;
        }
        // END temporary code
