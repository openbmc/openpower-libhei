...
def peltool_encode(model_ec: str, base: cd.Base, fp: object, extraSigPath: str):
    """
    ...
    if os.path.isfile(extraSigPath):
        with open(extraSigPath, "r") as sigFp:
            fileData = sigFp.read()
            sigPattern = 'PRD_EXTRA_SIG\s*\(\s*([^"]*)\s*,\s*(0x[0-9A-Fa-f]{8})\s*,\s*"([^"]*)"'
